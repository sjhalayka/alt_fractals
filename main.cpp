	...
	for (size_t x = 0; x < x_res; x++, Z.x += x_step_size)
	{
		...
		for (size_t y = 0; y < y_res; y++, Z.y += y_step_size)
		{
			...
			if (true == make_border && (x == 0 || y == 0 || z == 0 || x == x_res - 1 || y == y_res - 1 || z == z_res - 1))
				...
				xyplane0[x*y_res + y] = eqparser.iterate_len(p, Z, max_iterations, threshold);
				//xyplane0[x*y_res + y] = eqparser.iterate_disp(p, Z, max_iterations, threshold);
				//xyplane0[x*y_res + y] = eqparser.iterate_mag(p, Z, max_iterations, threshold);
		}
	}
		...
		for (size_t x = 0; x < x_res; x++, Z.x += x_step_size)
		{
			...
			for (size_t y = 0; y < y_res; y++, Z.y += y_step_size)
			{
				...
				if (true == make_border && (x == 0 || y == 0 || z == 0 || x == x_res - 1 || y == y_res - 1 || z == z_res - 1))
					...
					xyplane1[x*y_res + y] = eqparser.iterate_len(p, Z, max_iterations, threshold);
					//xyplane1[x*y_res + y] = eqparser.iterate_disp(p, Z, max_iterations, threshold);
					//xyplane1[x*y_res + y] = eqparser.iterate_mag(p, Z, max_iterations, threshold);
			}
		}
